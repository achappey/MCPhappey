...
public static class RunwayVideo
{
    ...
    [McpServerTool(Title = "Create Runway Text-to-Video", Name = "runway_text_to_video", OpenWorld = true, ReadOnly = false, Destructive = false)]
    ...
    [McpServerTool(Title = "Create Runway Image-to-Video", Name = "runway_image_to_video_create",
        OpenWorld = true,
        ReadOnly = false,
        Destructive = false)]
    public static async Task<CallToolResult?> Runway_ImageToVideo_Create(
        ...
        [Description("Ratio.")] string? ratio,
        int? duration,
        int? seed,
        IServiceProvider sp,
        RequestContext<CallToolRequestParams> rc,
        ...
        => await rc.WithExceptionCheck(async () =>
    {
        ...
        foreach (var img in promptImages)
        {
            ...
            var bytes = files.FirstOrDefault()?.Contents ?? throw new Exception($"Failed to download image: {img}");
            string base64 = Convert.ToBase64String(bytes.ToArray());
            string dataUri = $"data:image/png;base64,{base64}";

            imagePayload.Add(new { uri = dataUri, position = position == null ? "first" : "last" });
            position = "first";
        }
    ...
    public class RunwayNewImageToVideo
    {
        ...
        [JsonPropertyName("ratio")]
        [Required]
        [Description("Output ratio string.")]
        public string Ratio { get; set; } = default!;
        ...
        await rc.WithStructuredContent(async () =>
    {
        ...
        var refBytes = refFiles.FirstOrDefault()?.Contents ?? throw new Exception($"Failed to download reference video: {referenceUri}");
        string refBase64 = Convert.ToBase64String(refBytes.ToArray());
        string refDataUri = $"data:video/mp4;base64,{refBase64}";
        ...
        await rc.WithExceptionCheck(async () =>
    {
        ...
        var bytes = files.FirstOrDefault()?.Contents ?? throw new Exception($"Failed to download video: {videoUri}");
        string base64 = Convert.ToBase64String(bytes.ToArray());
         ...
         await rc.WithExceptionCheck(async () =>
    {
        ...
        // Encode source video
        var vidFiles = await downloadService.ScrapeContentAsync(sp, rc.Server, videoUri, ct);
        var vidBytes = vidFiles.FirstOrDefault()?.Contents ?? throw new Exception($"Failed to download video: {videoUri}");
        string vidBase64 = Convert.ToBase64String(vidBytes.ToArray());
        string vidDataUri = $"data:video/mp4;base64,{vidBase64}";

        // Encode reference image if provided
        object[]? references = null;
        if (!string.IsNullOrWhiteSpace(referenceImageUri))
        {
            var imgFiles = await downloadService.ScrapeContentAsync(sp, rc.Server, referenceImageUri, ct);
            var imgBytes = imgFiles.FirstOrDefault()?.Contents ?? throw new Exception($"Failed to download reference image: {referenceImageUri}");
            string imgBase64 = Convert.ToBase64String(imgBytes.ToArray());
            string imgDataUri = $"data:image/png;base64,{imgBase64}";
            references = [new { type = "image", uri = imgDataUri }];
        }
