...
public static class RunwayImages
{
        ...
        => await rc.WithExceptionCheck(async () =>
    {
        ...
        foreach (var img in promptImages)
        {
            ...
            imagePayloads.Add(new { uri = $"data:image/png;base64,{base64}" });
        }

        var payload = new
        {
            promptImages = imagePayloads.Count == 1 ? imagePayloads[0] : imagePayloads,
            promptText = typed.PromptText,
            model = typed.Model.GetEnumMemberValue(),
            ratio = typed.Ratio,
            seed = typed.Seed
        ...
        var json = await runway.ImageToImageAsync(payload, ct);
        var taskId = runway.ExtractTaskId(json);
