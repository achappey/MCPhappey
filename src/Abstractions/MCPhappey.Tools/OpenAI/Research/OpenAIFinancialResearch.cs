...
public static class OpenAIFinancialResearch
{
    ...
    private const string PlannerModel = "gpt-5";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            ["report_markdown"] = JsonSerializer.SerializeToElement((writerSample?.Content as TextContentBlock)?.Text ?? string.Empty),
            ["query"] = JsonSerializer.SerializeToElement(topic)
        };
        ...
        // Output
        var finalText = BuildFinalText((writerSample?.Content as TextContentBlock)?.Text,
        (verification?.Content as TextContentBlock)?.Text, fundamentalsText, riskText, searchResults!);
        ...
        CancellationToken cancellationToken)
    {
        ...
        return (sample?.Content as TextContentBlock)?.Text;
    }
