...
import { McpServerWithName } from "../../types/mcp"; // Corrected import path
...
const MCPServerModal: React.FC<MCPServerModalProps> = ({ show, server, onClose }) => {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      ...
      try {
        if (server && !connected[server.url]) {
          // Ensure we connect if not already connected, using the server object
          // The connect function in context now internally uses server.url
          await connect(server); 
        }
        // Retrieve client using server.url
        const client = server ? getClient(server.url) : undefined; 
