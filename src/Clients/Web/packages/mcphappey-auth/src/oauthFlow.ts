...
export const initiateOAuthFlow = async (mcpServerUrl: string): Promise<void> => {
  ...
  if (!prResponse.ok) throw new Error(`Failed to fetch protected resource metadata (${prResponse.status}) from ${protectedResourceUrl}`);
  ...
  if (!asResponse.ok) throw new Error(`Failed to fetch authorization server metadata (${asResponse.status}) from ${authServerMetadataUrl}`);
...
export const handleOAuthCallback = async (): Promise<
  { accessToken: string; targetUrl?: string } | { error: string; errorDescription?: string }
> => {
