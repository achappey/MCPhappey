...
McpClientOptions clientOptions = new()
{
    ...
    Capabilities = new()
    {
        Sampling = !string.IsNullOrEmpty(settings?.OpenAI_ApiKey) ? new ModelContextProtocol.Protocol.SamplingCapability()
        {
            SamplingHandler = async (request, progress, cancellationToken) =>
            {
             ...
             Enumerable.Select<ModelContextProtocol.Protocol.SamplingMessage, OpenAI.Chat.ChatMessage>(
                ...
                return new ModelContextProtocol.Protocol.CreateMessageResult()
                {
                    Content = new()
                    {
