...
{
    Task<ClaimsPrincipal?> ValidateAsync(string token, string issuer, string audience, OAuthSettings oAuthSettings);
}

public class JwtValidator(IHttpClientFactory httpClientFactory) : IJwtValidator
{
    public async Task<ClaimsPrincipal?> ValidateAsync(string token, string issuer, string audience, OAuthSettings oAuthSettings)
    {
