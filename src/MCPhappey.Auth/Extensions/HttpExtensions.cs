...
public static class HttpExtensions
{
  ...
  {
    var token = serviceProvider.GetRequiredService<TokenProvider>();
    var outerJwt = new JwtSecurityTokenHandler().ReadJwtToken(token.Token);
  ...
  public static async Task<string> GetOboToken(this IHttpClientFactory httpClientFactory,
    ...
    OAuthSettings oAuthSettings)
  {
    if (server.Metadata?.ContainsKey(host) == true)
    {
      var delegated = await httpClientFactory.ExchangeOnBehalfOfTokenAsync(token,
                  ...
                  server.Metadata.GetScopes(host)?.ToArray() ?? []);
