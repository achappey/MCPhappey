...
var items = await httpClient.GetFromJsonAsync<MCPServerList>(settings?.MCPServerBackend);

var mcpServers = settings?.Servers == null || settings?.Servers?.Any() == false ?
    items?.McpServers : items?.McpServers.Where(a => settings!.Servers.Contains(a.Key));
...
foreach (var item in mcpServers ?? [])
{
    ...
    try
    {
        ...
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.Unauthorized)
        {
            ...
            var clientItem = await OAuthHandler.OAuthFallback(item.Value, clientOptions, settings!);
